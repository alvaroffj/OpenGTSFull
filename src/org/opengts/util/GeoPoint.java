    ...
    implements Cloneable, GeoPointProvider
{
    ...
    public GeoPoint(String gp, char sep)
    {
        ...
        if (gp != null) {
            ...
            if (p >= 0) {
                ...
                {
                    double lat = 0.0;
                    String latStr = gp.substring(0,p);
                    int d = latStr.indexOf(DegreeChar);
                    if (d > 0) {
                        double deg = StringTools.parseDouble(latStr.substring(0,d), 0.0);
                        double min = StringTools.parseDouble(latStr.substring(d+1), 0.0);
                        double sec = 0.0; // TODO
                        double sgn = (deg >= 0.0)? 1.0 : -1.0;
                        if (latStr.endsWith("S")) { sgn = -sgn; }
                        lat = (Math.abs(deg) + ((min + (sec / 60.0)) / 60.0)) * sgn;
                    } else {
                        lat = StringTools.parseDouble(latStr,0.0);
                    }
                ...
                {
                    double lon = 0.0;
                    String lonStr = gp.substring(p+1);
                    int d = lonStr.indexOf(DegreeChar);
                    if (d > 0) {
                        double deg = StringTools.parseDouble(lonStr.substring(0,d), 0.0);
                        double min = StringTools.parseDouble(lonStr.substring(d+1), 0.0);
                        double sec = 0.0; // TODO
                        double sgn = (deg >= 0.0)? 1.0 : -1.0;
                        if (lonStr.endsWith("W")) { sgn = -sgn; }
                        lon = (Math.abs(deg) + ((min + (sec / 60.0)) / 60.0)) * sgn;
                    } else {
                        lon = StringTools.parseDouble(lonStr,0.0);
                    }
    ...
    *** @param pp     The GeoPoint to convert to a pixel location
    ...
    **/
    protected static String _formatCoord(double loc, int fmt, Locale locale)
    {
        ...
        if ((fmt & FORMAT_TYPE_MASK) == FORMAT_NMEA) {
            ...
            sb.append(StringTools.format(100.0+min,"0.0000").substring(1));
            sb.append(",");
            if ((fmt & FORMAT_AXIS_MASK) == FORMAT_LATITUDE) {
                sb.append((sgn >= 0)? "N" : "S");
            } else
    ...
    **/
    public static void main(String argv[])
    {
        ...
        if (gp.isValid()) {
            ...
            Print.sysPrintln("8-byte encoded/decoded GeoPoint: " + gp8);
            encodeGeoPoint(gp, gpEnc, 0, 6);
            GeoPoint gp6 = decodeGeoPoint(gpEnc, 0, 6);
            Print.sysPrintln("6-byte encoded/decoded GeoPoint: " + gp6);
            System.exit(0);
        }
