    ...
    extends Thread
{
    ...
    *** @param command The line to write to the socket output stream. A newline is
    ***   appended if it does not already end with one.
    *** @throws IOException if an error occurs
    **/
    public void socketWriteLine(String command)
        throws IOException
    {
    ...
    *** @throws IOException if an error occurs
    **/
    protected static void socketWriteLine(Socket socket, String command)
        throws IOException
    {
        String cmd = command.endsWith("\n")? command : (command + "\n");
        ClientSocketThread.socketWriteBytes(socket, StringTools.getBytes(cmd), 0, -1);
    }
