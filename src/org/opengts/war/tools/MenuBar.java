                        ...
                        if (inclAnchor) {
                            ...
                            if (target.startsWith("_")) {
                                ...
                                if (pixDim != null) {
                                    int W = pixDim.getWidth();
                                    int H = pixDim.getHeight();
                                    out.write(" onclick=\"javascript:openFixedWindow('"+url+"','"+target+"',"+W+","+H+")\"");
                                    out.write(" style=\"text-decoration: underline; color: blue; cursor: pointer;\"");
                                } else {
                                    out.write("<a class='"+CSS_MENU_LINK+"' href='"+url+"' target='"+target+"'>");
                                }
                            }
                            out.write(desc);
                            out.write("</a>");
        ...
        if (privLabel.getBooleanProperty(PrivateLabel.PROP_MenuBar_usePullDownMenus,DFT_USE_PULL_DOWN_MENUS)) {
            ...
            if (mainMenu != null) {
                ...
                if ((pageName == null) || !pageName.equals(Track.PAGE_MENU_TOP)) {
                    //String url = WebPageAdaptor.EncodeMakeURL(reqState,Track.BASE_URI(),Track.PAGE_MENU_TOP);
