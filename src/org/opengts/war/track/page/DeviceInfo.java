    ...
    implements Constants
{
  ...
  //public  static final String PARM_WORKORDER_ID           = "d_workid";
    ...
    private static void _initDeviceCommandHandlers()
    {
        ...
        /* add available DeviceCmdHandler UI pages */
        ...
        String devCmdPackage = DeviceCmd_gtsdmtp.class.getPackage().getName();
        for (String dchCN : optDevCmdHandlerClasses) {
            String dchClassName = devCmdPackage + "." + dchCN;
            try {
                Class dch = Class.forName(dchClassName);
                _addDeviceCommandHandler((DeviceCmdHandler)dch.newInstance());
                //Print.logInfo("Added DeviceCmdHandler UI: " + dchClassName);
    ...
    */
    ...
    {
        int andMode = NEWDEV_ALLOW; // default "true"
        if (!Account.isSystemAdmin(currAcct)) {
            String globAND = privLabel.getStringProperty(PrivateLabel.PROP_DeviceInfo_allowNewDevice,"");
            if (StringTools.isBoolean(globAND,true)) {
                andMode = StringTools.parseBoolean(globAND,true)? NEWDEV_ALLOW : NEWDEV_DENY;
            } else {
                andMode = globAND.equalsIgnoreCase("sysadmin")? NEWDEV_SYSADMIN : NEWDEV_ALLOW;
            }
        }
        return andMode;
    }
