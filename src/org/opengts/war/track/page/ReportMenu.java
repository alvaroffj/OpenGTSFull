    ...
    implements Constants
{
    ...
    public  static final String  PARM_EMAIL_ADDR                = "r_emailAddr";
        ...
        throws IOException
    {
        ...
        /* HTML URL */
        String htmlURL   = Track.GetBaseURL(reqState); // EncodeMakeURL(reqState,Track.BASE_URI());
        ...
        out.write("   if (document."+FORM_SELECT_REPORT+"."+PARM_REPORT+".length) {\n");
        out.write("     var rc = document."+FORM_SELECT_REPORT+"."+PARM_REPORT+".length;\n");
        out.write("     for (var i = 0; i < rc; i++) {\n");
        out.write("       if (document."+FORM_SELECT_REPORT+"."+PARM_REPORT+"[i].checked) {\n");
        out.write("           return document."+FORM_SELECT_REPORT+"."+PARM_REPORT+"[i].value;\n");
        out.write("       }\n");
        out.write("     }\n");
        out.write("     return '?';\n");
        out.write("   } else {\n"); // assume that there is at least 1 report in the list
        out.write("     return document."+FORM_SELECT_REPORT+"."+PARM_REPORT+".value;\n");
        out.write("   }\n");
        out.write("}\n");
        ...
        out.write("   return document."+FORM_GET_REPORT+"."+PARM_FORMAT+".value;\n");
        out.write("}\n");

        out.write("// report format \n");
        out.write("function rptGetToEMailAddress() {\n");
        out.write("   try { return document."+FORM_GET_REPORT+"."+PARM_EMAIL_ADDR+".value; } catch(e) { return ''; }\n");
        out.write("}\n");
      ...
      //out.write("   alert('Report Option: ' + rptOption);\n");
        ...
        out.write("   if (outFmt == '"+ReportPresentation.FORMAT_CSV+"') {\n");
        ...
        out.write("   } else\n");
        out.write("   if (outFmt == '"+ReportPresentation.FORMAT_TXT+"') {\n");
        out.write("       var rptEnc = rptName.replace(/\\./g,'_');\n"); // <-- change '\\' to '\' when moving to a .js file
        out.write("       document."+FORM_COMMAND+".action = ReportHtmlURL + '_' + rptEnc + '.txt';\n"); // POST
        out.write("   } else\n");
        out.write("   if (outFmt == '"+ReportPresentation.FORMAT_XML+"') {\n");
        out.write("       var rptEnc = rptName.replace(/\\./g,'_');\n"); // <-- change '\\' to '\' when moving to a .js file
        out.write("       document."+FORM_COMMAND+".action = ReportHtmlURL + '_' + rptEnc + '.xml';\n"); // POST
        out.write("   } else\n");
        out.write("   if (outFmt == '"+ReportPresentation.FORMAT_CUSTOM+"') {\n");
        ...
        out.write("   document."+FORM_COMMAND+"."+parm_RANGE_FR    +".value = "+CALENDAR_FROM+".getArgDateTime();\n");
        out.write("   document."+FORM_COMMAND+"."+parm_RANGE_TO    +".value = "+CALENDAR_TO+".getArgDateTime();\n");
        out.write("   document."+FORM_COMMAND+"."+parm_TIMEZONE    +".value = calGetTimeZone();\n");
        out.write("   document."+FORM_COMMAND+"."+PARM_LIMIT       +".value = rptmGetLimit();\n");
        out.write("   document."+FORM_COMMAND+"."+PARM_LIMIT_TYPE  +".value = rptmGetLimitType();\n"); // not used
        out.write("   document."+FORM_COMMAND+"."+PARM_FORMAT      +".value = outFmt;\n");
        out.write("   document."+FORM_COMMAND+"."+PARM_EMAIL_ADDR  +".value = rptGetToEMailAddress();\n");
        out.write("   document."+FORM_COMMAND+"."+PARM_REPORT      +".value = rptName;\n");
        ...
        String xmlMsg   = i18n.getString("ReportMenu.xmlFormat"  ,"Report will be returned as an XML file");
        ...
        out.write("   var toEmailElem = document.getElementById('"+PARM_EMAIL_ADDR+"');\n");
        out.write("   if (toEmailElem) { toEmailElem.style.visibility = 'hidden'; }\n");
        out.write("   var fmtSelElem = document.getElementById('"+PARM_FORMAT+"');\n");
        ...
        out.write("       if (selVal == '"+ReportPresentation.FORMAT_CSV+"') {\n");
        out.write("           formatMsgElem.innerHTML = '"+csvMsg+"';\n");
        out.write("       } else\n");
        out.write("       if (selVal == '"+ReportPresentation.FORMAT_TXT+"') {\n");
        out.write("           formatMsgElem.innerHTML = '"+csvMsg+"';\n");
        out.write("       } else\n");
        out.write("       if (selVal == '"+ReportPresentation.FORMAT_XML+"') {\n");
        out.write("           formatMsgElem.innerHTML = '"+xmlMsg+"';\n");
        out.write("       } else\n");
        out.write("       if (selVal == '"+ReportPresentation.FORMAT_EHTML+"') {\n");
        ...
        out.write("         var rc = document."+FORM_SELECT_REPORT+"."+PARM_REPORT+".length;\n");
        out.write("         for (var i = 0; i < rc; i++) {\n");
        out.write("            var rptName = document."+FORM_SELECT_REPORT+"."+PARM_REPORT+"[i].value;\n");
        out.write("            var rptChkd = document."+FORM_SELECT_REPORT+"."+PARM_REPORT+"[i].checked;\n");
        ...
        throws IOException
    {
        ...
        final String parm_RANGE_TO = useMapDates? Calendar.PARM_RANGE_TO : Calendar.PARM_RANGE_FR2;
        ...
        String format    = (String)AttributeTools.getRequestAttribute(request, PARM_FORMAT    , ReportPresentation.FORMAT_HTML);
        ...
        AttributeTools.setSessionAttribute(request, parm_TIMEZONE, tzStr);
        ...
        AttributeTools.setSessionAttribute(request, parm_RANGE_FR, Calendar.formatArgDateTime(dateFr));
        AttributeTools.setSessionAttribute(request, parm_RANGE_TO, Calendar.formatArgDateTime(dateTo));
        ...
        AttributeTools.setSessionAttribute(request, PARM_LIMIT_TYPE, limitType); // not used
        AttributeTools.setSessionAttribute(request, PARM_FORMAT    , format);
        ...
        final String toEmailAddress = StringTools.trim(account.getReportEmail(user));
        final boolean hasToEmail;
        ...
        HTMLOutput HTML_CONTENT = new HTMLOutput(CommonServlet.CSS_CONTENT_FRAME, m) {
            public void write(PrintWriter out) throws IOException {
                String pageName  = ReportMenu.this.getPageName();
                String reportURL = Track.GetBaseURL(reqState); // EncodeMakeURL(reqState,Track.BASE_URI());
                ...
                out.write(" <input type='hidden' name='"+PARM_REPORT            +"' value=''/>\n");
                out.write(" <input type='hidden' name='"+PARM_REPORT_OPT        +"' value=''/>\n");
                out.write(" <input type='hidden' name='"+PARM_REPORT_TEXT       +"' value=''/>\n");
                out.write(" <input type='hidden' name='"+parm_RANGE_FR          +"' value=''/>\n");
                out.write(" <input type='hidden' name='"+parm_RANGE_TO          +"' value=''/>\n");
                out.write(" <input type='hidden' name='"+parm_TIMEZONE          +"' value=''/>\n");
                out.write(" <input type='hidden' name='"+PARM_LIMIT             +"' value=''/>\n");
                out.write(" <input type='hidden' name='"+PARM_LIMIT_TYPE        +"' value=''/>\n"); // not used
                out.write(" <input type='hidden' name='"+PARM_FORMAT            +"' value=''/>\n");
                out.write(" <input type='hidden' name='"+PARM_EMAIL_ADDR        +"' value=''/>\n");
                ...
                if (showTimezoneSelect) {
                    ...
                    out.println(  "<select name='"+parm_TIMEZONE+"' onchange=\"javascript:calSelectTimeZone(document.TimeZoneSelect."+parm_TIMEZONE+".value)\">");
                ...
                for (int t = 0; t < ReportFactory.REPORT_TYPES.length; t++) {
                    ...
                    if (reportItems.size() > 0) {
                        ...
                        for (Iterator<ReportEntry> i = reportItems.iterator(); i.hasNext();) {
                            ...
                            out.write("<input class='"+CSS_REPORT_RADIO_BUTTON+"' type='radio' name='"+PARM_REPORT+"' id='" + rn + "' value='" + FilterValue(rn) + "' onchange=\"javascript:rptmReportRadioChanged();\"");
                ...
                out.write("    <select id='"+PARM_FORMAT+"' name='"+PARM_FORMAT+"' onchange=\"javascript:rptmFormatChanged();\">\n");
                out.write("      <option value='"+ReportPresentation.FORMAT_HTML +"' selected>HTML</option>\n");
                out.write("      <option value='"+ReportPresentation.FORMAT_CSV  +"'>CSV</option>\n");
              //out.write("      <option value='"+ReportPresentation.FORMAT_TXT  +"'>TXT</option>\n");
                out.write("      <option value='"+ReportPresentation.FORMAT_XML  +"'>XML</option>\n");
                if (hasToEmail) {
                    out.write("      <option value='"+ReportPresentation.FORMAT_EHTML+"'>EMail</option>\n");
                }
                if (hasCustomURL) {
                    out.write("      <option value='"+ReportPresentation.FORMAT_CUSTOM+"'>Custom</option>\n");
                }
                out.write("    </select>\n");
                ...
                if (hasToEmail) {
                    ...
                    out.write("    <input class='"+emailClass+"' id='"+PARM_EMAIL_ADDR+"' name='"+PARM_EMAIL_ADDR+"' style='margin-top:5px; margin-left:10px;; visibility:hidden' type='text' "+emailRO+" value='"+toEmailAddress+"' size='76'>");
                } else {
                    out.write("    <input id='"+PARM_EMAIL_ADDR+"' name='"+PARM_EMAIL_ADDR+"' type='hidden' value=''/>\n");
                }
                out.write("    </form>\n");
                out.write("  </td>\n");
                out.write(" </tr>\n");
