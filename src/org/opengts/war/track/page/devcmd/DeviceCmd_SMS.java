    ...
    implements DeviceCmdHandler
{
    ...
    {
        if (!DCServerFactory.hasServerConfig(DEVICE_CODE)) {
        ...
        if (reqState == null) {
            ...
            if (!ListTools.isEmpty(this.getCommandMap(pl,user,DCServerFactory.CMDTYPE_ADMIN))) {
                return true;
            }
            if (!ListTools.isEmpty(this.getCommandMap(pl,user,DCServerFactory.CMDTYPE_SYSADMIN))) {
                return true;
            }
    ...
    **/
    private RTProperties sendCommand(Device device, DCServerConfig.Command command, String cmdArgs[])
    {
        ...
        RTProperties rtp = DCServerFactory.createRTProperties(acctID, devID, cmdType, cmdID, cmdArgs);
