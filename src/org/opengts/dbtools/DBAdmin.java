...
public class DBAdmin
{
    ...
    **/
    ...
    public static final int VALIDATE_CHECK_ENCODING         = 0x0008;   // 'u' (requires 'a' or 'w')
    ...
    **/
    private static boolean validateTableColumns(final int validateMask, String tableName)
    {
        ...
        if (!validatedColumns) {
            ...
            boolean alterCol = ((validateMask & VALIDATE_ALTER_COLUMNS ) != 0);
            if (addCol || alterCol) {
                ...
                Print.sysPrintln("Rerun with option '-tables=c' or '-tables=ca' (see usage).");
            }
        } else {
    ...
    **/
    ...
    public static DBAdminExec execCommands() // return
    {
        ...
        */
        ...
        File loadFile = null;
        String loadTable = RTConfig.getString(ARG_LOAD, null);
        if (loadTable != null) {
            execCmd++;
            if (StringTools.isBlank(loadTable)) {
                Print.logError("'Load' file not specified");
                return DBAdminExec.ERROR;
            }
            if (FileTools.hasFileSeparator(loadTable)) {
                // IE. "-load=./Geozone[.csv]"
                File file = new File(loadTable);
                loadTable = file.getName();
                loadDir = file.getParentFile(); // override any directory specified with '-dir=<dir>'
            }
            int p = loadTable.lastIndexOf('.');
            if (p < 0) {
                // IE. "Geozone"
                loadFile = new File(loadDir, loadTable + DBFactory._LOAD_EXT_TXT);
                if (!loadFile.isFile()) { 
                    loadFile = new File(loadDir, loadTable + DBFactory._LOAD_EXT_SQL);
                    if (!loadFile.isFile()) {
                        String f = loadDir + File.separator + loadTable + ".[txt|sql]";
                ...
                // IE. "Geozone.csv"
                String ext = loadTable.substring(p); // ==> ".csv"
                if (ext.equals(DBFactory._LOAD_EXT_CSV) || ext.equals(DBFactory._LOAD_EXT_TXT) || ext.equals(DBFactory._LOAD_EXT_SQL)) {
                    loadFile = new File(loadDir, loadTable);
                    if (!loadFile.isFile()) {
                        Print.logWarn("'Load' file not found: " + loadFile);
                        return DBAdminExec.WARN;
                    }
                } else {
                ...
                loadTable = loadTable.substring(0,p);
            }
        }
        // loadTable, loadFile, [loadDir] now defined ...
        ...
        // bin/exe DBAdmin -load=<Table> -dir=<Source_Dir>
        if (loadFile != null) {
            execCmd++;
            if (loadFile.isFile()) {
                ...
                if (fact != null) {
                    ...
                    try {
                        ...
                        Print.logInfo("Loading table '" + loadTable + "' from file: " + loadFile);
                        if (overwriteExisting) {
                            Print.logWarn("**** Existing data WILL be overwritten! ****");
                        } else {
                            Print.logWarn("---- Existing data will NOT be overwritten! ----");
                        }
                        long count = fact.loadTable(loadFile, overwriteExisting);
                        Print.logInfo("(Loaded " + count + " records from file '" + loadFile + "' into table '" + loadTable + "')");
                    } catch (DBException dbe) {
                        Print.logException("Error creating/loading table: " + loadTable, dbe);
                        return DBAdminExec.ERROR;
                    }
                } else {
                ...
                Print.logError("File not found: " + loadDir + File.separator + loadTable + ".[dump|sql|txt|csv]");
                return DBAdminExec.ERROR;
            }
        }
