    ...
    implements DBRecordListener<gDBR>
{
    ...
    **/
    public boolean validateColumns(int validateMask, boolean refreshColumns)
    {
        ...
        java.util.List<DBField> missingColumns = new Vector<DBField> ();
        java.util.List<DBField> typeMismatchColumns = new Vector<DBField>();
        for (int i = 0; i < colDefined.length; i++) {
            ...
            if (!DBProvider.areTypesEquivalent(definedType,actualType)) {
                ...
                if (displayErrors) {
                    Print.logInfo("ERROR - " + tblName + ": Column '" + columnName + "' Type mismatch '" + definedType + "' != '" + actualType + "'");
                }
                typeMismatchColumns.add(colDefined[i]);
                columnsOK = false;
            }
