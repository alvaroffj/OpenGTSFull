    ...
    implements UserInformation
{
    ...
    public  static String           DEMO_ACCOUNT_ID         = "demo";
    public  static String           DEMO_DEVICE_ID_1        = "demo";
    public  static String           DEMO_DATE_RANGE_1[]     = new String[] { "2010/03/12", "2010/03/12" };
    public  static String           DEMO_DEVICE_ID_2        = "demo2";
    public  static String           DEMO_DATE_RANGE_2[]     = new String[] { "2010/03/12", "2010/03/12" };
    
    public static boolean IsDemoAccount(String accountID)
    {
        return Account.DEMO_ACCOUNT_ID.equals(accountID);
    }

    // ------------------------------------------------------------------------
    // Temporary account attributes

    public  static final long       MAX_EXPIRATION_SEC      = DateTime.DaySeconds(7);
  ...
  //public static final String FLD_newUserRoleID            = "newUserRoleID";
    public static final DBField FieldInfo[] = {
      ...
      //new DBField(FLD_privateLabelJsp     , String.class  , DBField.TYPE_STRING(32)  , "PrivateLabel JSP"          , "edit=2"),
        ...
        // User fields
        new DBField(FLD_defaultUser         , String.class  , DBField.TYPE_STRING(32)  , "Default User ID"           , "edit=2"),
    ...
    {
        this.setFieldValue(FLD_defaultUser, ((v != null)? v : ""));
    }
    ...
    public static String encodePassword(String userPass)
    {
        // all passwords are encodable (even if it's a 1-1 encoding)
        return Account.getPasswordHandler().encodePassword(userPass);
    }
    ...
    public void setCreationDefaultValues()
    {
        ...
        if (BasicPrivateLabelLoader.isPrivateLabelAdminAccountID(this.getAccountID())) {
            // see also createNewAccount(String acctID, String passwd, String privateLabelName)
            this.setDescription("New PrivateLabel Account");
            BasicPrivateLabel bpl = BasicPrivateLabelLoader.getPrivateLabelForSysAdminAccountID(this.getAccountID());
            this.setPrivateLabelName(bpl.getDomainName());
        } else {
            ...
            //this.setGeocoderMode((rgp != null)? GeocoderMode.FULL : GeocoderMode.GEOZONE);
            this.setGeocoderMode(GeocoderMode.FULL); // <-- default to FULL for now
            //this.setIsBorderCrossing(Account.SupportsBorderCrossing());
            this.setIsBorderCrossing(true); // <-- default to True for now
        }
    ...
    **/
    public String getReportEmail(User user)
    {
        
        /* try user email address */
        if ((user != null) && user.getAccountID().equals(this.getAccountID())) {
            String userEmail = user.getContactEmail();
            if (!StringTools.isBlank(userEmail)) {
                return userEmail;
            }
        }
        ...
        try {
            ...
            if (adminUser != null) {
                String userEmail = adminUser.getContactEmail();
                if (!StringTools.isBlank(userEmail)) {
                    return userEmail;
                }
            }
    ...
    public long getDeviceCount()
    {
        try {
            ...
            String where = dwh.WHERE_(dwh.EQ(Device.FLD_accountID,this.getAccountID()));
        ...
        throws DBException
    {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                Print.logInfo("Found Device with recent notification: "+deviceID+" ==> "+notifyTime+":"+StatusCodes.GetHex(notifyCode));
                break;
            }
        ...
        throws DBException
    {
        ...
        /* PrivateLabel Admin account? */
        String accountID = account.getAccountID();
        if (account.isPrivateLabelAdmin()) {
            // guaranteed to have proper matching PrivateLabelName
            String acctPrivateLabelName = account.getPrivateLabelName();
            DBSelect<Account> dsel = new DBSelect<Account>(Account.getFactory());
            dsel.setSelectedFields(Account.FLD_accountID);
            dsel.setOrderByFields(Account.FLD_accountID);
            DBWhere dwh = dsel.createDBWhere();
            dsel.setWhere(dwh.WHERE(dwh.EQ(Account.FLD_privateLabelName, acctPrivateLabelName)));
            return Account.getAllAccounts(dsel);
        }

        /* only authorized to self */
        Collection<String> acctList = new Vector<String>();
        acctList.add(accountID);
        return acctList;
    ...
    *** @return The created/retrieved Account (does not return null)
    *** @throws A DBException if the account already exists and 'create' was specified,
    ...
    **/
    ...
    /**
    *** Creates a new Account with the specified ID.  The password will be blank,
    *** and the Accunt admin user will not be able to log in until a valie password
    *** has bee assigned.
    *** @param acctID  The account ID to create
    *** @return The created account
    *** @throws DBException if an error occurs, or if account already exists
    **/
    public static Account createNewAccount(String acctID)
        throws DBException
    {
        return Account.createNewAccount(acctID, null, null);
    }
    ...
    *** @throws DBException if an error occurs, or if account already exists
    **/
    public static Account createNewAccount(String acctID, String passwd)
        throws DBException
    {
        return Account.createNewAccount(acctID, passwd, null);
    }
    ...
    *** @param acctID  The account ID to create
    *** @param passwd  The account password
    *** @param privateLabelName  The PrivateLabel name
    *** @return The created account
    *** @throws DBException if an error occurs, or if account already exists
    **/
    public static Account createNewAccount(String acctID, String passwd, String privateLabelName)
        throws DBException
    {
        ...
        /* create account */
        Account acct = Account.getAccount(acctID, true); // does not return null

        /* set password */
        if (passwd != null) { // empty string allowed
            acct.setPassword(passwd);
        }

        /* set PrivateLabel name */
        if (acct.isPrivateLabelAdmin()) {
            if (!acct.getPrivateLabelName().equals(privateLabelName)) {
                Print.logWarn("Account '"+acctID+"' does not match specified PrivateLabelName: " + privateLabelName);
            }
        } else
        if (!StringTools.isBlank(privateLabelName)) {
            acct.setPrivateLabelName(privateLabelName);
        }
    ...
    **/
    public static Account createTemporaryAccount(String contactName, String contactEmail, String privateLabelName)
        throws DBException
    {
        ...
        long tval = (now % DateTime.DaySeconds(34)) / 3; // 0..979200
        ...
        synchronized (TempAccountLock) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                if (!acctKey.exists()) { // may throw DBException
                    ...
                    account.setPasswdQueryTime(now);
                    account.setPassword(Account.createRandomPassword(Account.TEMP_PASSWORD_LENGTH));
                    account.setExpirationTime(now + Account.MAX_EXPIRATION_SEC);
        ...
        if (RTConfig.getBoolean(ARG_CREATE, false) && hasAccountID) {
            ...
            if (accountExists) {
                ...
                try {
                    ...
                    Account acct = Account.createNewAccount(acctID, passwd);
