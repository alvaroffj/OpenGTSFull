    ...
    extends DBRecord<RT>
{
    ...
    **/
    ...
    /* return true if the specified account is a privateLabel admin */
    public static boolean isPrivateLabelAdmin(Account account)
    {
        if (account == null) {
            return false;
        } else {
            return account.isPrivateLabelAdmin();
        }
    }

    /* return true if this account is a privateLabel admin */
    public final boolean isPrivateLabelAdmin()
    {
        String accountID = this.getAccountID();
        BasicPrivateLabel bpl = BasicPrivateLabelLoader.getPrivateLabelForSysAdminAccountID(accountID);
        if (bpl != null) {
            String abpln = this.getAccount().getPrivateLabelName();
            if (bpl.getDomainName().equals(abpln)) {
                return true;
            } else {
                Print.logWarn("Account '"+accountID+"' is a PrivateLabelAdmin, but does not have the proper PrivateLabelName!");
                return false;
            }
        } else {
            return false;
