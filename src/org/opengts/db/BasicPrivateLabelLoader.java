...
public class BasicPrivateLabelLoader
{
    ...
    public  static final String TAG_AdministratorAccount        = "AdministratorAccount";
    ...
    **/
        ...
        boolean ignoreDuplicates)
    {
        ...
        BasicPrivateLabel.SetSupportedLocalesMap(this.parseTag_SupportedLocales(xmlFile, i18nPkgName, privLabels));
        ...
        for (int p = 0; p < propsNodes.getLength(); p++) {
            ...
            this.parseTag_Properties(xmlFile, globalProps, props, ""); // blank is significant
    ...
    **/
        ...
        boolean ignoreDuplicates)
    {
        ...
        for (int c = 0; c < attrList.getLength(); c++) {
            ...
            if (attrName.equalsIgnoreCase(TAG_AdministratorAccount)) {
                pl.setPrivateLabelAdminAccount(XMLTools.getNodeText(attrElem,"",true));
            } else
            ...
            if (attrName.equalsIgnoreCase(TAG_Properties)) {
                this.parseTag_Properties(xmlFile, pl, attrElem, null); // null is significant
    ...
    *** @param dftRTPrefix  The properties key "prefix"
    **/
    protected void parseTag_Properties(File xmlFile, RTConfig.PropertySetter ps, Element attrElem, String dftRTPrefix)
    {

        /* key prefix */
        String rtpfx     = StringTools.trim(XMLTools.getAttribute(attrElem,ATTR_rtPropPrefix,null,true));
        String keyPrefix = !StringTools.isBlank(rtpfx)? rtpfx : dftRTPrefix;
        ...
        for (int e = 0; e < propList.getLength(); e++) {
            ...
            String propName = propNode.getNodeName();
            if (propName.equalsIgnoreCase(TAG_Property)) {
                this.parseTag_Property(xmlFile, ps, propElem, keyPrefix);
            } else
            if (propName.equalsIgnoreCase(TAG_LogMessage)) {
                this.parseTag_LogMessage(xmlFile, null, propElem);
            } else {
    ...
    *** @param attrElem  The 'Property' node
    *** @param keyPrefix The key prefix used to lookup externally defined runtime values. 
    **/
    protected void parseTag_Property(File xmlFile, RTConfig.PropertySetter ps, Element attrElem, String keyPrefix)
    {
    ...
    **/
    protected void parseTag_ReverseGeocodeProvider(File xmlFile, BasicPrivateLabel pl, Element rgpAttrElem)
    {
        ...
        /* properties */
        String keyPrefix = XMLTools.getAttribute(rgpAttrElem,ATTR_rtPropPrefix,null,true);
        RTProperties rtProps = new RTProperties();
    ...
    **/
    ...
    /**
    *** Returns a Set of all private label PrivateLabelAdmin Account IDs
    *** @return A Set of all private label PrivateLabelAdmin Account IDs
    **/
    public static Set<String> getPrivateLabelAdminAccountIDs()
    {
        ...
        if (privLblMap != null) {
            for (Iterator<String> i = privLblMap.keySet().iterator(); i.hasNext();) {
                BasicPrivateLabel pbl = privLblMap.get(i.next());
                String plaa = pbl.getPrivateLabelAdminAccount();
                if (!StringTools.isBlank(plaa) && !idSet.contains(plaa)) {
                    idSet.add(plaa);
                }
            }
        }
        return idSet; // may be empty
    }

    /**
    *** Returns the BasicPrivateLabel for the specified PrivateLabel SystemAdmin Account ID
    *** @return The BasicPrivateLabel for the specified PrivateLabel SystemAdmin Account ID
    **/
    public static BasicPrivateLabel getPrivateLabelForSysAdminAccountID(String sysAdminID)
    {
        Map<String,BasicPrivateLabel> privLblMap = BasicPrivateLabelLoader._getInstance().getPrivateLabelMap();
        if ((privLblMap != null) && !StringTools.isBlank(sysAdminID)) {
            for (Iterator<String> i = privLblMap.keySet().iterator(); i.hasNext();) {
                BasicPrivateLabel bpl = privLblMap.get(i.next());
                if (bpl.isPrivateLabelAdminAccountID(sysAdminID)) {
                    return bpl;
                }
            }
        }
    ...
    /**
    *** Returns true if the specified account ID is the PrivateLabelAdmin AccountID
    *** @return True if the specified account ID is the PrivateLabelAdmin AccountID
    **/
    public static boolean isPrivateLabelAdminAccountID(String sysAdminID)
    {
        BasicPrivateLabel bpl = BasicPrivateLabelLoader.getPrivateLabelForSysAdminAccountID(sysAdminID);
        return (bpl != null);
    }
    ...
    **/
    protected static I18N.Text parseI18N(File xmlFile, String pkgName, String i18nKey, String dftStr, boolean showError)
    {
        ...
        if (SAVE_I18N_STRINGS && !StringTools.isBlank(i18nKey)) {
            ...
            if (!I18N_STRINGS_MAP.containsKey(text.getKey())) {
                I18N_STRINGS.add(text);
                I18N_STRINGS_MAP.put(text.getKey(), text);
                //printInfo("I18N: %s=%s", text.getKey(), text.getDefault());
            } else {
                printInfo("I18N: key already defined - " + text.getKey());
            }
        }
        return text;
